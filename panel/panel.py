...
def refresh_ssh_banner_config():
    ...
    lines = ["# Hotscript - Dynamic per-user SSH banners\n"]
    users = read_db()
    for u in users:
        un = u["username"]
...
# --- PANEL CREDS ---
def get_panel_creds():
    creds = {"PANEL_USER": "", "PANEL_PASS_HASH": "", "PANEL_PASS_PLAIN": "", "PANEL_SECRET": "", "PANEL_NAME": "Hotscript", "PANEL_LOGO": "🔥"}
...
def write_panel_creds(user, pass_plain, secret=None, panel_name=None, panel_logo=None):
    ...
    if panel_name is not None:
        creds["PANEL_NAME"] = panel_name.strip() or "Hotscript"
...
class PanelAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if api_path == "/api/branding":
                creds = get_panel_creds()
                self.send_json(200, {
                    "panel_name": creds.get("PANEL_NAME", "Hotscript"),
    ...
    def handle_put_settings(self, body):
        ...
        self.send_json(200, {"success": True, "secret": new_secret, "panel_name": new_name or creds.get("PANEL_NAME", "Hotscript"), "panel_logo": new_logo or creds.get("PANEL_LOGO", "🔥")})
