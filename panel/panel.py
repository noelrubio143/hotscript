...
def refresh_ssh_banner_config():
    ...
    lines = ["# Hot VPN - Dynamic per-user SSH banners\n"]
    users = read_db()
    for u in users:
        un = u["username"]
...
# --- PANEL CREDS ---
def get_panel_creds():
    creds = {"PANEL_USER": "", "PANEL_PASS_HASH": "", "PANEL_PASS_PLAIN": "", "PANEL_SECRET": "", "PANEL_NAME": "Hot VPN", "PANEL_LOGO": "🔥"}
...
def write_panel_creds(user, pass_plain, secret=None, panel_name=None, panel_logo=None):
    ...
    if panel_name is not None:
        creds["PANEL_NAME"] = panel_name.strip() or "Hot VPN"
...
class PanelAPIHandler(BaseHTTPRequestHandler):
    ...
    def _get_session(self):
        """Auth disabled: every request is treated as an authenticated admin session."""
        return {"username": "admin", "role": "admin", "created_at": time.time()}
    ...
    def do_GET(self):
        try:
            ...
            if api_path == "/api/branding":
                creds = get_panel_creds()
                self.send_json(200, {
                    "panel_name": creds.get("PANEL_NAME", "Hot VPN"),
    ...
    def do_DELETE(self):
        ...
        # Delete disabled in the panel: accounts must be removed from the
        # server terminal directly (e.g. `userdel`), not through the public panel.
        self.send_json(403, {"error": "Delete is disabled in the panel. Remove accounts from the server terminal instead."})
    ...
    def handle_protocol_restart(self, service):
        allowed = set()
        for p in PROTOCOLS:
            allowed.add(p["service"])
            if p.get("service_alt"):
                allowed.add(p["service_alt"])
        if service not in allowed:
            return self.send_json(400, {"success": False, "error": "Unknown or disallowed service"})
    ...
    def handle_put_settings(self, body):
        ...
        self.send_json(200, {"success": True, "secret": new_secret, "panel_name": new_name or creds.get("PANEL_NAME", "Hot VPN"), "panel_logo": new_logo or creds.get("PANEL_LOGO", "🔥")})
